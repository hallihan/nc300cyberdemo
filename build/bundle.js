...
function guessDevice(e) {
  if (e.device) return e.device;            // UAParser vendor + model, when offered
...
// Battery ascending; unknown battery sinks to the bottom, ordered by browser.
function sortEntries(rows) {
  return rows.sort(function (x, y) {
    const bx = batteryPct(x.battery), by = batteryPct(y.battery);
    if (bx === null && by === null) return (x.browser || "").localeCompare(y.browser || "");
    if (bx === null) return 1;
    if (by === null) return -1;
    return bx - by;
  });
...
const INFO_COLUMNS = [
  { label: "Device",    get: function (e) { return guessDevice(e); } },
  { label: "Battery",   get: function (e) { return e.battery || "n/a"; }, numeric: true },
  { label: "Dark Mode", get: function (e) { return e.darkMode || "n/a"; } },
  { label: "OS",        get: function (e) { return e.os || "n/a"; } },
  { label: "Browser",   get: function (e) { return e.browser || "n/a"; } },
  { label: "IP",        get: function (e) { return e.ip || "n/a"; } },
  { label: "ISP",       get: function (e) { return e.isp || "n/a"; } },
  { label: "Location",  get: function (e) { return e.location || "n/a"; } }
];

function renderInfo(host, countEl, entries) {
  const rows = sortEntries(Object.values(entries || {}));
  host.textContent = "";
  if (countEl) countEl.textContent = rows.length === 1 ? "1 device" : rows.length + " devices";

  if (!rows.length) {
    const empty = document.createElement("div");
    empty.className = "info-empty";
    empty.textContent = "No devices captured yet.";
  ...
  INFO_COLUMNS.forEach(function (col) {
    const th = document.createElement("th");
    th.textContent = col.label;
    if (col.numeric) th.className = "info-num";
  ...
  rows.forEach(function (entry) {
    ...
    INFO_COLUMNS.forEach(function (col) {
      ...
      td.textContent = col.get(entry);
      if (col.numeric) td.className = "info-num";
      tr.appendChild(td);
    });
...
function pt(t) {
  let btn, sp1, wrap, sp2, count, mounted = !1, dispose;
  return {
    c() {
      btn = document.createElement("button");
      btn.textContent = "Clear Info";
      btn.className = "button-p svelte-bm01p3";
      btn.style.fontSize = "0.7em";
      btn.style.marginTop = "15px";
      sp1 = document.createTextNode(" ");
      wrap = document.createElement("div");
      wrap.className = "info-wrap";
      sp2 = document.createTextNode(" ");
      count = document.createElement("div");
      count.className = "info-count";
      renderInfo(wrap, count, t[1]);
    },
    m(target, anchor) {
      i(target, btn, anchor);
      i(target, sp1, anchor);
      i(target, wrap, anchor);
      i(target, sp2, anchor);
      i(target, count, anchor);
      mounted || (dispose = m(btn, "click", t[15]), mounted = !0);
    },
    p(ctx, dirty) {
      if (2 & dirty) renderInfo(wrap, count, ctx[1]);
    },
    d(detaching) {
      detaching && l(btn);
      detaching && l(sp1);
